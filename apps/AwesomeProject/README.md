...
## Monorepo workspace (`aepsdk-react-native`)

This app lives under `apps/AwesomeProject` and is a **Yarn workspace** of the parent repo. Adobe React Native packages (`@adobe/react-native-aep*`) resolve to **`../../packages/*`** (local source), not the npm registry.

- From the **repository root**: run `yarn install` (if the lockfile changes, use `YARN_ENABLE_IMMUTABLE_INSTALLS=false yarn install` once).
- Build JS for packages: `yarn build` (TypeScript `dist/` for all `packages/*`).
