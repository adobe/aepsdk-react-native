...
| 2026-06-08 | Migrated event emission from `RCTDeviceEventEmitter` / `NativeEventEmitter` to `CodegenTypes.EventEmitter<PropositionsPayload>`. Added `readonly onPropositionsUpdated: CodegenTypes.EventEmitter<PropositionsPayload>` to `specs/NativeAEPOptimize.ts`. iOS: `RCTAEPOptimize.h` now inherits `NativeAEPOptimizeSpecBase` on both paths (provides `emitOnPropositionsUpdated:`). Android: `NativeAEPOptimizeModule.java` uses `emitOnPropositionsUpdated(payload)` with `{propositions: ...}` wrapper. JS: unified single subscription path — `NativeAEPOptimize.onPropositionsUpdated(callback)` — no `Platform.OS` branch, no `NativeEventEmitter`. See `context/turbo-module-event-emission.md`. |
| 2026-06-08 | Merged main into branch. Removed `react-native`/`react` from root `devDependencies`; fixed `jest.config.js` `moduleNameMapper` to resolve RN from `AwesomeProject/node_modules`. Added Metro `blockList` + `extraNodeModules` to `apps/AEPSampleAppNewArchEnabled/metro.config.js` to pin RN and `@react-navigation/*` versions. Added `resolutions` to `apps/AEPSampleAppNewArchEnabled/package.json` to deduplicate `@react-navigation` nested copies from expo-router. See known-gotchas #24. |
